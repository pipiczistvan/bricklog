...
class WatchCurrencyPreferenceDetails(
    ...
    operator fun invoke(): Flow<CurrencyPreferenceDetails> {
        return watchUserPreferences()
            .flatMapLatest { userPreferences ->
                setRepository.watchPriceRanges(userPreferences.currencyRegion)
                    .map { userPreferences to it }
            }.flatMapLatest { (userPreferences, priceRanges) ->
